...
- **UI Integration**: Full API for Auxin-App
...
├── Resources/
│   └── com.auxin.daemon.plist    # LaunchAgent config
└── Tests/
...
# Binary location: .build/release/auxin-daemon
```

### Installation Steps

#### Method 1: Via Auxin App (Recommended)
...
sudo cp .build/release/auxin-daemon /usr/local/bin/

# 3. Install LaunchAgent plist
mkdir -p ~/Library/LaunchAgents
cp Resources/com.auxin.daemon.plist ~/Library/LaunchAgents/
...
File: `~/Library/LaunchAgents/com.auxin.daemon.plist`
...
1. Start daemon: auxin-daemon --daemon
2. Open Logic Pro project
3. Make changes and save
4. Wait 30 seconds
5. Verify commit: auxin log
...
# View error logs
cat /tmp/com.auxin.daemon.stderr
...
launchctl list | grep com.auxin.daemon.xpc
...
AUXIN_LOG=debug auxin-daemon --daemon

# Run under lldb
lldb .build/debug/auxin-daemon
(lldb) run --daemon

# Attach to running daemon
sudo lldb -p $(pgrep auxin-daemon)
```
...
4. Update `Auxin-App` to use new method
5. Add tests
