...
fn test_lock_collision() {
    ...
    2. oxenvcs-cli lock acquire --timeout 4
    3. Verify success

    Machine B:
    1. cd test-project.logicx
    2. oxen pull origin locks
    3. oxenvcs-cli lock acquire --timeout 4
    4. EXPECTED: Failure - lock held by Machine A

    Machine A:
    1. oxenvcs-cli lock release

    Machine B:
    1. oxenvcs-cli lock acquire --timeout 4
...
fn test_complete_collaboration_workflow() {
    ...
    3. oxenvcs-cli lock acquire --timeout 8
    4. # Make changes to project
    5. oxenvcs-cli add --all
    6. oxenvcs-cli commit -m 'Recorded vocals' --bpm 120 --tags recording
    7. oxen push origin main
    8. oxenvcs-cli lock release

    === User B (Mixer) ===
    1. oxenvcs-cli activity --limit 10  # See A's work
    2. oxenvcs-cli team                 # Check team stats
    3. oxen pull origin main            # Get A's changes
    4. oxenvcs-cli lock acquire --timeout 4
    5. # Make changes
    6. oxenvcs-cli add --all
    7. oxenvcs-cli commit -m 'Mixed vocals' --bpm 120 --tags mixing
    8. oxenvcs-cli comment add <commit> 'Great vocal take!'
    9. oxen add .oxen/comments/ && oxen commit -m 'Add comment'
    10. oxen push origin main
    11. oxenvcs-cli lock release

    === User A ===
    1. oxen pull origin main
    2. oxenvcs-cli activity --limit 10  # See B's work
    3. oxenvcs-cli comment list <commit> # See B's comment
...
fn test_large_project_performance() {
    ...
    2. Initialize: oxenvcs-cli init --logic .
    3. Add: oxenvcs-cli add --all
    4. Commit: oxenvcs-cli commit -m 'Initial' --bpm 120
