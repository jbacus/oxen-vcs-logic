...
High-performance Rust CLI and library for Oxen VCS operations on Logic Pro projects. This component provides the core VCS functionality for the OxVCS system with minimal latency and low resource usage.
...
# The binary will be at: target/release/oxenvcs-cli
```
...
export PATH="/path/to/OxVCS-CLI-Wrapper/target/release:$PATH"

# Option 3: Use cargo install (from crate)
cargo install oxenvcs-cli
```
...
oxenvcs-cli restore <commit-hash>
...
oxenvcs-cli init --logic . --draft-branch my-drafts
...
oxenvcs-cli log --with-metadata

# Status with porcelain output (machine-readable)
oxenvcs-cli status --porcelain

# Force restore (discard uncommitted changes)
oxenvcs-cli restore --force <commit-hash>
```
...
oxenvcs-cli add --all
oxenvcs-cli commit -m "WIP: working on arrangement"

# Switch to main for milestone commits
oxenvcs-cli checkout main
oxenvcs-cli merge draft
oxenvcs-cli commit -m "Version 1.0 - Ready for mixing" --tags "milestone"

# Return to draft for continued work
oxenvcs-cli checkout draft
```
...
export OXENVCS_LOG=debug

# Custom draft branch name
export OXENVCS_DRAFT_BRANCH=my-drafts

# Maximum draft commits before pruning
export OXENVCS_MAX_DRAFT_COMMITS=100
```
...
oxenvcs-cli add Resources/Audio_01.wav
```
...
```swift
// Swift code in OxVCS-LaunchAgent
let process = Process()
process.executableURL = URL(fileURLWithPath: "/usr/local/bin/oxenvcs-cli")
...
```bash
OXENVCS_LOG=debug oxenvcs-cli status
```
