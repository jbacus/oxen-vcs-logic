/// Daemon client for communicating with OxVCS LaunchAgent
///
...
const LAUNCH_AGENT_LABEL: &str = "com.oxenvcs.agent";
...
impl DaemonClient {
    ...
    pub fn log_path(&self) -> Result<String> {
        let home = std::env::var("HOME").context("HOME environment variable not set")?;
        Ok(format!("{}/Library/Logs/OxVCS/daemon.log", home))
    }
...
mod tests {
    ...
    fn test_extract_pid() {
        let output = r#"{
    "Label" = "com.oxenvcs.agent";
    "LimitLoadToSessionType" = "Aqua";
    "OnDemand" = false;
    "LastExitStatus" = 0;
    "PID" = 12345;
    "Program" = "/usr/local/bin/oxvcs-daemon";
};"#;
    ...
    fn test_log_path() {
        ...
        if let Ok(path) = client.log_path() {
            assert!(path.contains("Library/Logs/OxVCS"));
        }
    }
}
