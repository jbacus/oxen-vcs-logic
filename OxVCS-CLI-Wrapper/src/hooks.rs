/// Workflow automation hooks for OxVCS
///
...
const HOOKS_README: &str = r#"# OxVCS Hooks
...
if command -v osascript &> /dev/null; then
    osascript -e "display notification \"$OXVCS_MESSAGE\" with title \"OxVCS Commit\""
fi

# Example: Linux notification
if command -v notify-send &> /dev/null; then
    notify-send "OxVCS Commit" "$OXVCS_MESSAGE"
fi
