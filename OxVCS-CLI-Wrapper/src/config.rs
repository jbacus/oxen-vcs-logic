//! Configuration file support for OxVCS CLI
//!
...
impl Default for QueueConfig {
    fn default() -> Self {
        Self {
            auto_sync: true,
            queue_dir: "~/.oxenvcs/queue".to_string(),
            max_entries: 1000,
...
impl Config {
    ...
    /// Get user config path (~/.oxenvcs/config.toml)
    pub fn user_config_path() -> Option<PathBuf> {
        dirs::home_dir().map(|home| home.join(".oxenvcs").join("config.toml"))
    }

    /// Get project config path (.oxenvcs/config.toml in current directory)
    pub fn project_config_path() -> Option<PathBuf> {
        std::env::current_dir()
            .ok()
            .map(|cwd| cwd.join(".oxenvcs").join("config.toml"))
    }
    ...
    fn apply_env_vars(mut config: Config) -> Config {
        // OXENVCS_VERBOSE
        if let Ok(val) = std::env::var("OXENVCS_VERBOSE") {
        ...
        // OXENVCS_COLOR
        if let Ok(val) = std::env::var("OXENVCS_COLOR") {
        ...
        // OXENVCS_LOCK_TIMEOUT
        if let Ok(val) = std::env::var("OXENVCS_LOCK_TIMEOUT") {
        ...
        // OXENVCS_MAX_RETRIES
        if let Ok(val) = std::env::var("OXENVCS_MAX_RETRIES") {
        ...
        // OXENVCS_QUEUE_DIR
        if let Ok(val) = std::env::var("OXENVCS_QUEUE_DIR") {
