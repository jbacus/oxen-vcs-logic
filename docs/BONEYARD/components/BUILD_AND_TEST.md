# Auxin App - Build and Test Guide
...
The app requires the Auxin-LaunchAgent to be running for full functionality.
...
ls -lh .build/release/Auxin

# Expected output:
# -rwxr-xr-x  1 user  staff   XXX KB  Date  Auxin
```
...
# In Xcode:
# 1. Select "Auxin" scheme (top left)
...
# Or from Finder: Applications → Auxin
```
...
launchctl list | grep com.auxin
# Should show: com.auxin.agent with a PID
...
- [ ] **Test 1.3:** Daemon status updates
  ...
  - If red, check: `launchctl list | grep com.auxin`
...
- [ ] **Test 6.1:** Open settings
  1. Press ⌘, or menu: Auxin → Preferences
...
- [ ] **Test 10.1:** Daemon not running
  1. Stop daemon: `launchctl unload ~/Library/LaunchAgents/com.auxin.agent.plist`
  2. Launch app
...
swift test --filter Auxin-AppTests

# Generate code coverage (requires Xcode)
swift test --enable-code-coverage

# View coverage report
xcodebuild test -scheme Auxin -enableCodeCoverage YES
```
...
cd ../Auxin-LaunchAgent
.build/release/Auxin-LaunchAgent &
...
defaults delete com.oxen.logic.Auxin
```
...
3. **Runtime Logs:**
   ```bash
   # App logs
   .build/debug/Auxin 2>&1 | tee run.log

   # Daemon logs
   log show --predicate 'process == "Auxin-LaunchAgent"' --last 1h > daemon.log
   ```
...
Submit to: https://github.com/jbacus/auxin/issues
...
2. **Notarize for Distribution** (Apple Developer Account required)
   ```bash
   # Sign the app
   codesign --deep --force --verify --verbose --sign "Developer ID Application: YOUR_NAME" /Applications/Auxin.app

   # Submit for notarization
   xcrun notarytool submit /Applications/Auxin.app --apple-id YOUR_EMAIL --password APP_SPECIFIC_PASSWORD --team-id YOUR_TEAM_ID
   ```

3. **Create DMG Installer**
   ```bash
   # Use create-dmg or similar tool
   create-dmg --volname "Auxin Installer" --window-size 600 400 --icon Auxin.app 200 200 --app-drop-link 400 200 Auxin-Installer.dmg /Applications/Auxin.app
   ```
