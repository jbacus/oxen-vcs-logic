...
impl AuthManager {
    ...
    /// Fallback: Encrypted storage in ~/.auxin/credentials (not implemented yet)
    ///
    /// # Arguments
    ///
...
mod tests {
    ...
    fn test_auth_manager_new() {
        let auth = AuthManager::new();
        assert!(auth.config_file.to_str().unwrap().contains(".auxin"));
    }
    ...
    fn test_default_config_path() {
        ...
        assert!(path.to_str().unwrap().contains(".auxin"));
